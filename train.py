...
tf.flags.DEFINE_float("learning_rate", 1e-2, "Which learning rate to start with.")
...
def make_batches(train_sent1_word_index, train_sent1_dist_index, train_trigger1_word_index, train_trigger1_dist_index, train_sent2_word_index, train_sent2_dist_index, train_trigger2_word_index, train_trigger2_dist_index, label, trigger_common,time_diff ):
    batches = []
    n = len(label)
    print(n)
    for i in range(0, n, FLAGS.batch_size):
        batch = []
        batch.append(train_sent1_word_index[i:min(i+FLAGS.batch_size-1, n-1)])
        batch.append(train_sent1_dist_index[i:min(i+FLAGS.batch_size-1, n-1)])
        batch.append(train_trigger1_word_index[i:min(i+FLAGS.batch_size-1, n-1)])
        batch.append(train_trigger1_dist_index[i:min(i+FLAGS.batch_size-1, n-1)])
        batch.append(train_sent2_word_index[i:min(i+FLAGS.batch_size-1, n-1)])
        batch.append(train_sent2_dist_index[i:min(i+FLAGS.batch_size-1, n-1)])
        batch.append(train_trigger2_word_index[i:min(i+FLAGS.batch_size-1, n-1)])
        batch.append(train_trigger2_dist_index[i:min(i+FLAGS.batch_size-1, n-1)])
        batch.append(label[i:min(i+FLAGS.batch_size-1, n-1)])
        batch.append(trigger_common[i:min(i+FLAGS.batch_size-1, n-1)])
        batch.append(time_diff[i:min(i+FLAGS.batch_size-1, n-1)])
...
# TRAINING DATA
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            batches = make_batches(train_sent1_word_index, train_sent1_dist_index, train_trigger1_word_index, train_trigger1_dist_index, train_sent2_word_index, train_sent2_dist_index, train_trigger2_word_index, train_trigger2_dist_index, train_label,train_trigger_common,train_time_diff)
            print(len(baches))
            ##------------ TRAIN BATCHES --------------
            for i in range(0, 10):
...
def eval():
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            batches = make_batches(test_sent1_word_index, test_sent1_dist_index, test_trigger1_word_index, test_trigger1_dist_index, test_sent2_word_index, test_sent2_dist_index, test_trigger2_word_index, test_trigger2_dist_index, test_label,test_trigger_common,test_time_diff)
            all_predictions = []
            for batch in batches:
                feed_dict = {
